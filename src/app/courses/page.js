...
// import uniData from "@/data/universities_uk.json";
...
export default function Home() {
  ...
  const destinations = uniData.map((c) => ({
    name: c.country,
    value: c.country.toLowerCase().replace(/\s+/g, "-"),
    flag: "🌐",
  }));

  useEffect(() => {
    if (!selectedDestination || !selectedDegree) {
      setCurrentCountryCourses([]);
      return;
    }
    const countryObj = uniData.find(
      (c) => c.country.toLowerCase() === selectedDestination.replace(/-/g, " ")
    );
    if (!countryObj) {
      setCurrentCountryCourses([]);
      return;
    }
    const courses = Object.values(countryObj.universities)
      .flatMap((u) => Object.keys(u[selectedDegree] || {}))
      .filter((v, i, a) => a.indexOf(v) === i);
    setCurrentCountryCourses(courses);
  }, [selectedDestination, selectedDegree]);
  ...
  const handleCourseInputChange = (e) => {
    ...
    if (value.length > 0) {
      setFilteredSuggestions(
        currentCountryCourses.filter((s) =>
